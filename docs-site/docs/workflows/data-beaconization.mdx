...
Validate Beacon XLSX or JSON input and write BFF JSON collections
...
Valid worksheets become deterministic BFF JSON collections. Use `--no-color` or `--no-emoji` when plain output is required for a log processor.
...
Raw VCF input must run annotation, which is enabled by default. This error usually means `--no-annotate` or `annotate: false` was used on an unannotated file. Remove that override and supply the [annotation configuration](../getting-started/annotation-data).
...
<details>
<summary>SnpEff tries to download a database</summary>

The local genome database is missing or `data.dir` in `snpEff.config` points to the host path rather than the path visible inside the container. Set it to the mounted bundle path, for example `/beacon2-cbi-tools-data/databases/snpeff/v5.0`.
...
Click any row to open the variant inspector without leaving the table. It groups the clinical interpretation, variant definition, external evidence links, matched panels, and carrier genotypes while preserving the current filters and page.

![BFF Tools Browser variant inspector showing a pathogenic variant, condition identifiers, and its variant definition](/img/bff-variant-detail-inspector.png)
...
MongoDB clients are external to the application and Docker image. The current compatibility archive may still contain legacy copies, but `bff-tools` does not use them; the next versioned annotation bundle will omit them.
