...
:::tip[Starting from an existing spreadsheet or clinical format?]
The workbook above is designed for metadata already arranged in Beacon entity worksheets. For a regular Excel table, export the relevant worksheet as CSV and use [Convert-Pheno](https://github.com/CNAG-Biomedical-Informatics/convert-pheno) to map its columns to BFF. Convert-Pheno also supports sources such as REDCap, OMOP CDM, Phenopackets v2, FHIR, and CDISC. Validate the generated BFF collections with `bff-tools validate` before loading them into a Beacon.
:::

```text title="Successful metadata validation"
🧬 BFF Tools Validator  v2.0.13
...
:::warning[`--no-annotate` requires biallelic input]
This option skips bcftools normalization as well as annotation. The input must already contain **one ALT allele per record**. Split multiallelic records before conversion; otherwise genotype allele indexes and allele-specific annotations cannot be mapped reliably.
:::
...
Resource versions materially affect output and must be recorded with every annotation-enabled run. Complete setup and the packaged compact integration test are documented under [Annotation Data](../getting-started/annotation-data).
